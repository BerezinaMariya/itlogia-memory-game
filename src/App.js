...
class App extends React.Component {
    ...
    constructor() {
        super();
        this.state = {cards: [], clicks: 0, seconds: 0, isPopupOpened: false};
    }
    ...
    startGame() {
        this.setState({
            cards: this.prepareCards(), clicks: 0, seconds: 0, isPopupOpened: false
        });
    ...
    checkForAllCompleted() {
        if (this.state.cards.every(item => item.isCompleted)) {
            ...
            if (!!localStorage.getItem('time')) {
                if (parseInt(localStorage.getItem('time')) > this.state.seconds) {
                    alert('Вы установили новый рекорд времени!');
            ...
            <div className="App">
                ...
                <Popup open={this.state.isPopupOpened} closeOnDocumentClick onClose={this.closePopup.bind(this)}>
                    <div className="modal">
                        ...
                        Игра завершена! Ваш результат: {this.state.clicks} нажатий!
                    </div>
                </Popup>
